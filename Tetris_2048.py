import numpy as np
import stddraw  # Basic Graphics Library
import random  # Used For Creating Random Shapes/Types
from game_grid import GameGrid  #Importing Game Grid Class
from tetromino import Tetromino  #Importing Tetrominoes Class
from picture import Picture  #Importing Picture Class Images to Display
import os  #File and Directory Operations
from color import Color  #Importing Color Class Coloring the Menu
...
class Game:
    ...
    def start(self):
        ...
        canvas_height, canvas_width = 40 * grid_height, 40 * grid_width
        stddraw.setCanvasSize(canvas_width, canvas_height)
        # Coordinate System
        stddraw.setXscale(-0.5, grid_width - 0.5)
        stddraw.setYscale(-0.5, grid_height - 0.5)

        # Creates a blank list to keep 10 tetrominos
        self.tetrominos = list()
        # Round Count
        self.round_count = 0
        # Creates 10 tetrominos and assigns them to the self.tetrominos array
        self.create_tetromino(grid_height, game_w)
        ...
        self.display_menu(grid_height, grid_width, grid)
        ...
        while True:
            ...
            # Checking If the User Interacted with the Keyboard
            if stddraw.hasNextKeyTyped():
                ...
                # Checks if Down is pressed
                    ...
                    # Moves the Tetromino
                    # Causing it to Fall Down Faster
                    current_tetromino.move(key_typed, grid)
                elif key_typed == "up":
            ...
            if not self.is_paused:
                # move the Tetromino down by 1 at each time
            ...
            if not success:
                ...
                merge_tile= self.check_merging(grid)
                while merge_tile:
                    merge_tile = self.check_merging(grid)

                # Keeps row informations if they are completely filled or not
                row_count = self.is_full(grid_height, grid_width, grid)
                index = 0
                ...
                labels, num_labels = self.connected_component_labeling(grid.tile_matrix, grid_width, grid_height)
                free_tiles = [[False for v in range(grid_width)] for b in range(grid_height)]
                free_tiles, num_free = self.find_free_tiles(grid_height, grid_width, labels, free_tiles)
                ...
                new_x_cor, new_y_cor = random.randint(2, 9), 21
                current_tetromino.move_pos(new_x_cor, new_y_cor)
            ...
            if self.restart:
                ...
                current_tetromino = self.tetrominos[self.round_count]
                grid.current_tetromino = current_tetromino
                new_x_cor, new_y_cor = random.randint(2, 9), 22
                current_tetromino.move_pos(new_x_cor, new_y_cor)
    ...
    # If row is completely filled it takes True value
    def is_full(self, grid_height, grid_width, grid):
        # Creates an array with full of False, array size is equal to number of rows in the game grid
        row_count = [False for i in range(grid_height)]
    ...
    def create_tetromino(self, grid_height, grid_width):
        self.rotated = False
        # Shape of the Tetromino is Determined Randomly in the Tetromino Type Array
        tetromino_shapes = ['I', 'O', 'Z', 'J', 'L', 'T', 'S']
        for i in range(10):
            random_index = random.randint(0, len(tetromino_shapes) - 1)
            self.random_type = tetromino_shapes[random_index]
    ...
    def display_menu(self, grid_height, grid_width, grid):
        # Color used for background
        background_color = Color(0, 0, 0)
        # Color used for button
        button_color = Color(31, 160, 239)
        # Color used for text
        text_color = Color(255 , 255, 255)
        # Clear the Background Canvas to Background Color
        stddraw.clear(background_color)
        # get the directory in which this python code file is placed
        current_dir = os.path.dirname(os.path.realpath(__file__))
        # path of the image
        image_file = current_dir + "/menu_image.png"
        # Display Image in the Center
        image_center_x, image_center_y = (grid_width - 1) / 2, grid_height - 7
        # Image is represented using the Picture class
        image_to_display = Picture(image_file)
        # Display the image
        stddraw.picture(image_to_display, image_center_x, image_center_y)
        # Dimensions of the Start Game Button
        button_w, button_h = grid_width - 9.5, 2
        # coordinates of the bottom left corner of the start game button
        button_blc_x, button_blc_y = image_center_x - button_w / 2, 4
        ...
        if not self.is_finished and self.is_paused:
            stddraw.setPenColor(button_color)
            button2_blc_x, button2_blc_y = image_center_x - button_w / 2, 1
            ...
            stddraw.text(image_center_x, 5, text_to_display)

            text1_to_display = "Restart"
            stddraw.text(image_center_x, 2, text1_to_display)
                ...
                if stddraw.mousePressed():
                    ...
                    if mouse_x >= button_blc_x and mouse_x <= button_blc_x + button_w:
                        ...
                        # Restart the game
                            ...
                            self.speed_screen(grid, background_color, grid_width, grid_height, image_file, button_color)
                            break

        # If game is finished, allows user to restart game
        elif self.is_finished:
            stddraw.setPenColor(Color(232, 38, 38))
            stddraw.text(image_center_x, 8, "Game Over")
            stddraw.setPenColor(text_color)
            text1_to_display = "Restart"
            stddraw.text(image_center_x, 5, text1_to_display)
            while True:
                stddraw.show(50)
                if stddraw.mousePressed():
                    # X and Y Coordinates of the location at which the mouse has
                    # most recently been left-clicked
                    mouse_x, mouse_y = stddraw.mouseX(), stddraw.mouseY()
                    if mouse_x >= button_blc_x and mouse_x <= button_blc_x + button_w:
                        if mouse_y >= button_blc_y and mouse_y <= button_blc_y + button_h:
                            ...
                            # Allows user to choose game speed
                            self.speed_screen(grid, background_color, grid_width, grid_height, image_file, button_color)
                            break
    ...
    def speed_screen(self, grid, background_color, grid_width, grid_height, image_file, button_color):
        stddraw.clear(background_color)
        ...
        stddraw.setPenColor(button_color)
        # Coordinates of the bottom left corner of the start game button
        button1_blc_x, button1_blc_y = image_center_x - button_w / 2, 4
        ...
        stddraw.text(image_center_x, 5, normal_speed_text)

        fast_speed_text = "Slow"
        stddraw.text(image_center_x - 5, 5, fast_speed_text)

        text_to_display = "Fast"
        stddraw.text(image_center_x + 5, 5, text_to_display)
    ...
    def connected_component_labeling(self, grid, grid_width, grid_height):
    ...
    def find_free_tiles(self, grid_heigth, grid_width, labels, free_tiles):
        counter = 0
        okay_labels = []
        for x in range(grid_heigth):
